<template lang="pug">
  div
    span.delete-btn.position-absolute
      i.material-icons(style="color: #ccc; font-size: 1.3rem;" :name="name" @click="$emit('deleteImage', $event)") cancel
    label(:for="guid")
      img.img-fluid(v-if="imgBlob" :src="imgBlob")
      ImgFirebaseLoader.img-fluid(v-else-if="imgPath && imgPath !== ''" :path="imgPath" objectFit="cover" :modalDisabled="true")
      img.img-fluid(v-else src="~assets/noimage.jpg") 
