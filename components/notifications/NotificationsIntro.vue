<template lang="pug">
.w-100.mt-1.mb-1.border-bottom(v-if="isShow && isLoading")
  .header-line
    div.content {{ content() }}
    div.close-icon
      i.material-icons.icon(@click="clickClose") close
...
export default {
  ...
  methods: {
    ...
    async clickClose() {
      await this.$store.dispatch("notifications/delete", this.notification.id);
    },
    linkTo() {
      if (this.notification.kind === this.ADMIN_NOTIFICATION)
        return this.notification.details[0].content;
      return "#";
    },
    content() {
      if (this.notification.kind === this.ADMIN_NOTIFICATION)
      ...
      if (this.notification.kind === this.USER_FOLLOW) {
        const followUser = this.userList.filter(user => user.id === this.notification.userId)
        return followUser[0].name + " あなたをフォローしました";
      }
