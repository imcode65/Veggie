<template lang="pug">
.mypage.bg-white(v-if="timeline")
  TimelineContentV2(:timeline="timeline")
  footer.row.text-right
    .col-md-6
    .col-md-6
      .row(style="padding: 0rem 1rem 0rem 1rem")
        TimelineLikeBtn.icon.heart(style="justify-content: center;" :timeline="timeline")
        TimelineCommentBtn.icon.comment(style="justify-content: center;" :timeline="timeline")
        ...
        div.btn.col.d-flex.align-items-center.icon(style="justify-content: center;" )
    ...
    showPostModal() {
      if (this.$checkAuthenticated()) {
        this.$modal.show(
          ...
          {
            clickToClose: true,
	          width: $(window).width() < 850 ?  $(window).width() - 100 : 770,
            height: "800px",
            scrollable: true,
          }
        );
